...
import { OWNER_NAV, allTenants } from '../constants';
import { PaperAirplaneIcon } from '../components/Icons';
import type { Conversation, Message } from '../types';
import { useData } from '../contexts/DataContext';

const currentOwnerId = 'o002';
...
const OwnerMessages: React.FC = () => {
    ...
    const ownerConversation = useMemo(() => conversations.find(c => c.participantId === currentOwnerId), [conversations]);

    const [activeConversationId, setActiveConversationId] = useState<string | null>(ownerConversation?.id || null);
    ...
    const handleSendMessage = (e: React.FormEvent) => {
        ...
        const newMessageObj: Message = {
            ...
            sender: 'participant', // The owner is the participant in the data model
    ...
    return (
        <DashboardLayout navItems={OWNER_NAV} activePath="/owner/messages">
                    ...
                    <div className="flex-1 overflow-y-auto py-3">
                       <ul>
                            {ownerConversation ? (
                                <li>
                                    <button
                                        onClick={() => setActiveConversationId(ownerConversation.id)}
                                        className={`w-full text-left flex items-center p-3 rounded-lg transition-colors ${activeConversationId === ownerConversation.id ? 'bg-blue-100' : 'hover:bg-slate-100'}`}
                                    >
                                        ...
                                        <div className="flex-1 overflow-hidden">
                                            <div className="flex justify-between items-center">
                                                <p className={`font-semibold truncate ${activeConversationId === ownerConversation.id ? 'text-blue-800' : 'text-slate-800'}`}>Property Manager</p>
                                                <p className="text-xs text-slate-400 flex-shrink-0">{timeAgo(ownerConversation.lastMessageTimestamp)}</p>
                                            </div>
                                            <p className="text-sm text-slate-500 truncate">{ownerConversation.lastMessage}</p>
                                        </div>
                                    </button>
                                </li>
                            ) : null}

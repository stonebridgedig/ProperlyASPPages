...
const allReports = [
    ...
    { id: 'owner-statement', title: 'Owner Statement', description: 'A financial summary specifically formatted for property owners.', category: 'Financial', icon: DocumentIcon, isPopular: false },
    ...
    { id: 'maintenance-history', title: 'Maintenance History', description: 'Historical log of all completed work orders and costs.', category: 'Operations', icon: WrenchIcon, isPopular: false },
];
...
const Reports: React.FC = () => {
    // Alias `tenants` from context to `allTenants` locally to avoid naming conflicts with other variables in this component.
    const { 
        transactions, 
        chartOfAccounts, 
        properties, 
    ...
    const previewData = useMemo(() => {
        ...
        switch (selectedReport) {
            ...
            case 'Owner Statement':
                ...
                return { columns: ['Category', 'Amount'], data: [{ Category: 'Total Income', Amount: income }, { Category: 'Total Expenses', Amount: expense }, { Category: 'Net Operating Income', Amount: income - expense }]};
            case 'Tenant Directory':
                return { columns: ['Name', 'Email', 'Phone', 'Property', 'Unit', 'Status'], data: allTenants.filter(t => property === 'All Properties' || t.propertyName === property) };
            case 'Lease Expiration':
                return { columns: ['Tenant', 'Property', 'Unit', 'Lease End Date'], data: allTenants.filter(t => { if (t.status !== 'Active' || !t.leaseEndDate) return false; if (property !== 'All Properties' && t.propertyName !== property) return false; const leaseEndDate = new Date(t.leaseEndDate); return (!startDate || leaseEndDate >= startDate) && (!endDate || leaseEndDate <= endDate); }).map(t => ({ Tenant: t.name, Property: t.propertyName, Unit: t.unitName, 'Lease End Date': t.leaseEndDate })) };
            case 'Vacancy Report':
                 const vacancies = properties.flatMap(prop => prop.buildings.flatMap(b => b.units.filter(u => u.status === 'Vacant').map(u => ({ Property: prop.name, Unit: u.name, 'Market Rent': u.rent, Beds: u.bedrooms, Baths: u.bathrooms }))).filter(v => property === 'All Properties' || v.Property === property));
                return { columns: ['Property', 'Unit', 'Market Rent', 'Beds', 'Baths'], data: vacancies };
            case 'Open Maintenance':
                return { columns: ['Issue', 'Property', 'Unit', 'Priority', 'Status', 'Submitted'], data: allMaintenanceRequests.filter(req => req.status !== 'Completed' && (property === 'All Properties' || req.property === property)).map(r => ({ ...r, Submitted: new Date(r.submittedDate).toLocaleDateString() })) };
            case 'Maintenance History':
                 return { columns: ['Issue', 'Property', 'Unit', 'Priority', 'Status', 'Submitted'], data: allMaintenanceRequests.filter(req => (property === 'All Properties' || req.property === property) && (!startDate || new Date(req.submittedDate) >= startDate) && (!endDate || new Date(req.submittedDate) <= endDate)).map(r => ({ ...r, Submitted: new Date(r.submittedDate).toLocaleDateString() })) };
            default:
                return { columns: [], data: [] };
        }
    }, [selectedReport, filters, filteredTransactions, properties, allTenants, allMaintenanceRequests, rentRollData]);
            ...
            <div className="h-full flex flex-col">
                ...
                <div className="p-4 border-y border-slate-200 bg-white flex-shrink-0">
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div><label className="text-xs font-medium text-slate-500 mb-1 block">Property</label><select name="property" value={filters.property} onChange={handleFilterChange} className="w-full text-sm bg-white border border-slate-300 rounded-md px-3 py-2"><option>All Properties</option>{properties.map(p => <option key={p.name}>{p.name}</option>)}</select></div>
                        {selectedReport === 'Owner Statement' && (
                             <div><label className="text-xs font-medium text-slate-500 mb-1 block">Owner</label><select name="owner" value={filters.owner} onChange={handleFilterChange} className="w-full text-sm bg-white border border-slate-300 rounded-md px-3 py-2"><option value="all">All Owners</option>{allOwners.map(o => <option key={o.id} value={o.name}>{o.name}</option>)}</select></div>
                        )}
                    </div>
                </div>
                                            ...
                                            <tr key={index} className="hover:bg-slate-50 transition-colors">
                                                {previewData.columns.map(col => (
                                                    <td key={col} className="px-6 py-3 whitespace-nowrap text-slate-700">
                                                        {typeof row[col] === 'number' ? (col === 'Rent' || col === 'Balance' || col.includes('Amount') || col === 'Market Rent' ? `$${row[col].toLocaleString()}` : row[col]) : String(row[col])}
                                                    </td>
                                                ))}
                                            </tr>
                                        ))}
